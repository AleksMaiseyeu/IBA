...
Процесс преобразования большого числа блокировок уровня строки, страницы или индекса в одну блокировку уровня 
  таблицы называется эскалацией блокировок (lock escalation)
ALTER TABLE 
SET (LOCK_ESCALATION = {TABLE | AUTO | DISABLE})
Подсказки блокировок (locking hints) 
SET LOCK_TIMEOUT - период в миллисекундах, в течение которого транзакция будет ожидать снятия блокировки 
  с объекта (-1 по умолчанию, не установлен)

Взаимоблокировка (deadlock) — это особая проблема одновременного конкурентного доступа, в которой две транзакции 
...
* READ UNCOMMITTED
	...
	Допускает проблемы:
	 ...
	 REPEATABLE READ
	    Устанавливает разделяемые блокировки на все считываемые данные и удерживает эти блокировки до тех пор, 
		  пока транзакция не будет подтверждена или отменена
	 ...
	 SERIALIZABLE
		...
		Предотвращает вставку новых строк другой транзакцией до тех пор, пока первая транзакция не будет 
...
Проблемы:
	...
	Неповторяемое чтение
		Один процесс считывает данные несколько раз, а другой процесс изменяет эти данные между двумя операциями 
		  чтения первого процесса
...
declare @flag int = 2;
set implicit_transactions ON ---говорим отом, что обязательно нужно подтверждение
insert weather values('o|O');
if @flag = 1 commit else rollback; -- вставка срабатывает, если флаг = 1
